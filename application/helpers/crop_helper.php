...
if(!function_exists('advanced_crop')){
	...
	function advanced_resize($image_data,$n_w,$n_h,$suffix){
		...
		if($n_r!=$s_r){
			...
			//kalau ratio sama maka tidak dicrop, sehingga belum ada file baru untuk sauce resize
		...
		$config['height'] = $n_h;
		$ci->image_lib->initialize($config);
		$resizestatus=$ci->image_lib->resize();
		if (!$resizestatus){
			$message .= $ci->image_lib->display_errors();
		}
		$ci->image_lib->clear();
		$report['status']=$cropstatus&&$resizestatus;
